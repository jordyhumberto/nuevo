<?php
    ...
    $id = $_GET['IDSemestre'];
    $where = "";
    $sql = "SELECT * FROM tbl_det_semestre1 WHERE IDSemestre = '$id'";
...
<html lang="es">
	...
	<body>
		<div class="container">
			...
			<form class="form-horizontal" method="POST" action="m_g_d_update.php" autocomplete="off">
					...
					<div class="col-sm-10">
						<select class="form-control" id="id" name="id">
							<?php while($row = $resultado7->fetch_array(MYSQLI_ASSOC)) { ?>
								<option value="<?php echo $row['IDSemestre']; ?>"
                                <?php if($row['IDSemestre']==$fila['IDSemestre']) echo 'selected';?>
                ...
                <div class="form-group">
					...
					<div class="col-sm-10">
						<input type="date" class="form-control" id="fpago" name="fpago" placeholder="Fecha de pago" 
                        value="<?php echo $fila['Fecha_pago'];?>"
                        required>
					</div>
				</div>
